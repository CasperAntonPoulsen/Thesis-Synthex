...
def main():
    ...
    models = [i for i in models if i.endswith(".keras")]

    data_dir = "/dtu/p1/johlau/Thesis-Synthex/data/"
    output_dir = "/dtu/p1/johlau/Thesis-Synthex/predictions/"

    chexpert_test = pd.read_csv(data_dir+ "chexpert_test.csv")
    ...
    for file in tqdm(chexpert_test["image_path"]):
        image = keras.utils.img_to_array(keras.utils.load_img(data_dir + "chexpert-cropped/" + file))
    ...
    for name in models:
        ...
        pred_df.to_json(output_dir + "chexpert_" + name.replace(".keras", ".json"))
