...
input_dir = "/dtu/p1/johlau/Thesis-Synthex/data/QaTa-COV19/QaTa-COV19-v1/Images/"
output_dir =  "/dtu/p1/johlau/Thesis-Synthex/data/QaTa-COV19-CROPPED/"
image_paths = os.listdir(input_dir)

file_sizes = [(i,os.path.getsize(input_dir+i)) for i in image_paths]
df = pd.DataFrame(file_sizes, columns=["file_name","file_size"])
largest_file = list(df.sort_values(by="file_size", ascending=False).iloc[:50]["file_name"])

for idx, path in tqdm(enumerate(largest_file)):
